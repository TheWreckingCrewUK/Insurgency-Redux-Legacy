...
{
	if (((side _x) == west) && (alive _x) && ((player distance _x) < _val)) then {
		_return = _x;
		_val = player distance _x;
	};
...
if ((_return == player) && (count allplayers > 1)) exitwith {
	hint "Thats weird, the system cant find the nearest blufor player";
};
...
_textdis = ((((round (_val / 100))) * 100) + ((floor random 6) * 100) - 300);

_textdis1 = "about " + str _textdis;
if (_textdis < 1000) then {
	_textdis1 = "less than 1K";
};

_title  = "<t color='#ffbf00' size='1.2' shadow='1' shadowColor='#000000' align='center'>COALITION FORCES INTEL</t>"; 

_text1 = format ["<br />The nearest BLUFOR unit is %1M to your %2.<br /><br />", _textdis1, _textdir];
_text2 = "";
