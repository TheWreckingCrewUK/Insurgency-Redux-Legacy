...
if (count (_pos nearentities ["car", 200]) >_playercount) exitwith {hint "there are already enough vehicles here"};
if (count (_pos nearobjects ["Vysilacka", 200]) == 0) exitwith {hint "There's no forward base here"};

_vehicle = createvehicle ["UK3CB_BAF_LandRover_Soft_Sand_A", [0,0,0]];
_total = ((ceil(_playercount * 1.3))-(count (_pos nearentities ["car", 200]))) max 1;
