...
{if ((_x distance pythontruck) > 20) then {_r = "M_Scalpel_AT" createVehicle position _x;_r setVectorDirAndUp [[0,-1.5,-2],[-1,-1,0]]}; deletevehicle _x;
...
[] spawn {sleep 10; _pelpos = getpos pythontruck vectoradd [0, 5 * (cos (getdir pythontruck )), 2 * (tan (getdir pythontruck ))];   
   
 _pellet = createvehicle ["rhs_weap_9K115_launcher_used", _pelpos, [], 0, "can_collide"];    
  _ropepos = (pythontruck modeltoworld [0,10,0]); _rope = ropeCreate [pythontruck, [0.77,3.1,2], _pellet, [0, 0, 0], 150];    
 _pellet setvelocity [25 * (sin (getdir pythontruck )), 25 * (cos (getdir pythontruck )), 20]; sleep 10;{if ((_x distance pythontruck) > 20) then {_r = "M_Scalpel_AT" createVehicle position _x;_r setVectorDirAndUp [[0,-1.5,-2],[-1,-1,0]]}; deletevehicle _x; 
