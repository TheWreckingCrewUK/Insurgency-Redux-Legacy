...
twc_crashdetect = {
	sleep 20;
	if (diag_fps < 10) then {
		_var = missionnamespace getvariable ["twc_crashcounter", 0];
		_var = _var + 1;
		twc_crashcounter = _var;
		if (_var > 2) exitwith {"crash" remoteExec ["BIS_fnc_endMissionServer", 2];};
	} else {
		twc_crashcounter = 0;
	};
	[] spawn twc_crashdetect;
};
